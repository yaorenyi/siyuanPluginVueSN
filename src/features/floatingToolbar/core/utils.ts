 ...
 * @param options 消息选项
 */
export function showMessage(message: string, options: MessageOptions = {}): void {
...
export function showI18nMessage(
    ...
    const floatingToolbar = i18n?.floatingToolbar as Record<string, string> | undefined
...
export function getSelectedBlockId(): string | null {
    // 策略1: 获取多选块
    const selectedBlock = document.querySelector('.protyle-wysiwyg--select[data-node-id]')
    if (selectedBlock) {
