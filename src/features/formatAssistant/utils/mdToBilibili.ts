...
function applyInlineStyles(html: string, colors: BilibiliThemeColors, fontSize: number, lineHeight: number): string {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const borderStyle = i === 1
      ...
      : i === 2
        ? `border-bottom: 1px solid ${colors.hrColor}; padding-bottom: 4px;`
        : ""
  ...
  result = result.replace(
    /<img src="([^"]*)" alt="([^"]*)"[^/]*\/?>/g,
    `<img src="$1" alt="$2" style="max-width: 100%; border-radius: 8px; margin: 8px 0;" />`,
  )

  // blockquote
  result = result.replace(
    /<blockquote>/g,
    `<blockquote style="margin: 12px 0; padding: 10px 14px; border-left: 4px solid ${colors.quoteBorderColor}; background-color: ${colors.quoteBgColor}; color: ${colors.textColor}; font-size: ${fontSize}px; line-height: ${lineHeight}; border-radius: 0 8px 8px 0;">`,
  )

  // pre (代码块)
  result = result.replace(
    /<pre>/g,
    `<pre style="margin: 12px 0; padding: 0; background-color: ${colors.codeBgColor}; border-radius: 8px; overflow-x: auto; line-height: 1.5;">`,
  )

  // code with class (代码块内的 code)
  result = result.replace(
    /<code class="language-(\w+)">/g,
    (_, lang) => `<code class="language-${lang}" style="display: block; white-space: pre; overflow-x: auto; padding: 14px; font-family: 'Menlo', 'Monaco', 'Consolas', monospace; font-size: ${Math.round(fontSize * 0.85)}px; color: ${colors.textColor}; background: none; border-radius: 0 0 8px 8px;">`,
  )
  ...
  result = result.replace(
    /<table>/g,
    `<table style="width: 100%; border-collapse: collapse; margin: 12px 0; font-size: ${fontSize}px; border-radius: 8px; overflow: hidden;">`,
  )
