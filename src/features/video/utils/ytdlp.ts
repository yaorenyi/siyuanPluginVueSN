...
function getYtdlpPath(): string {
    ...
    for (const path of windowsPaths) {
      try {
        ...
        const expandedPath = path.replace(
          "%USERNAME%",
          (window as any).require("os").userInfo().username,
        )
  ...
  for (const path of unixPaths) {
    try {
      ...
      const expandedPath = path.replace(
        "~",
        (window as any).require("os").homedir(),
      )
      if (fs.existsSync(expandedPath)) {
        return expandedPath
      }
  ...
  try {
    ...
    return new Promise((resolve) => {
      ...
      process.stdout.on("data", (data: Buffer) => {
        ...
        if (downloadMatch) {
          const progress = Number.parseFloat(downloadMatch[1])
      ...
      process.on("close", (code: number) => {
        if (code === 0) {
          // 尝试从输出中提取文件名
          if (!fileName) {
            const destinationMatch = outputBuffer.match(
...
export async function getVideoInfo(url: string): Promise<{
  ...
  try {
    ...
    const args = [url, "--dump-json", "--no-playlist", "--skip-download"]

    const output = execSync(`${ytdlpPath} ${args.join(" ")}`, {
      encoding: "utf8",
    })
