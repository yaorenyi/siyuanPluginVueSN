 ...
 */
export async function mergeVideos(params: MergeVideosParams): Promise<FFmpegResult> {
  ...
  try {
    ...
    const fs = (window as any).require('fs')
    const listContent = absoluteVideoPaths.map(absolutePath => {
      // 转义单引号并用单引号包裹路径
 ...
 * 合并视频和音频文件
 */
export async function mergeVideoAudio(params: MergeVideoAudioParams): Promise<FFmpegResult> {
 ...
 * 压缩视频文件
 */
export async function compressVideo(params: CompressVideoParams): Promise<FFmpegResult> {
