 ...
 */
const extractBlockIdFromTypes = (
	types: readonly string[],
): { blockId: string | null; isBlockDrag: boolean } => {
	for (const type of types) {
		// 匹配思源块拖拽的类型格式
		const blockIdMatch = type.match(
			/application\/siyuan-gutternodeparagraph\S+(\d{14}-[a-z0-9]+)/,
		);
		if (blockIdMatch) {
 ...
 */
const extractBlockIdFromItems = async (
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (item.kind === "string") {
			try {
				...
				const idMatch =
					data.match(/"id":"([^"]+)"/) || data.match(/data-node-id="([^"]+)"/);
				if (idMatch) {
					return idMatch[1];
				}
			} catch (error) {
