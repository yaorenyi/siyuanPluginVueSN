      ...
      <div class="gp-dialog-body">
        <div class="gp-form-group">
          ...
          <div class="gp-path-row">
            ...
            />
            <button class="vp-btn vp-btn--ghost vp-btn--sm" @click="$emit('pick-scan-dir')">
        ...
        <div v-if="results.length > 0" class="gp-scan-results">
          <div class="gp-scan-results-header">
            <span class="gp-scan-count">{{ i18n.scanResults || '扫描结果' }} ({{ results.length }})</span>
            <button class="vp-btn vp-btn--ghost vp-btn--sm" style="font-size:10px;" @click="$emit('toggle-select-all')">
              {{ i18n.selectAll || '全选' }}
            </button>
          </div>
          <div v-for="repo in results" :key="repo.path" class="gp-scan-item">
        ...
        <div v-else-if="!scanning && localScanDir.trim() && results.length === 0" class="gp-empty" style="padding:20px 0;">
