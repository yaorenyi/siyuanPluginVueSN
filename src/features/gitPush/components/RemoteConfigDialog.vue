  ...
  <div class="gp-mask" @click.self="$emit('close')">
    <div class="gp-dialog" style="width: 420px;">
      ...
      <div class="gp-dialog-body">
        <div v-if="remotes.length" class="gp-remote-list">
          <div v-for="r in remotes" :key="r.name" class="gp-remote-row">
            <span class="gp-remote-name">{{ r.name }}</span>
            <template v-if="editingRemoteName === r.name">
              <input v-model.lazy="editingRemoteUrl" class="gp-input" style="flex:1" @keyup.enter="saveRemoteEdit(r.name)" @keyup.escape="editingRemoteName = ''" />
              <button class="vp-btn vp-btn--primary vp-btn--sm" :disabled="!editingRemoteUrl.trim()" @click="saveRemoteEdit(r.name)">保存</button>
              <button class="vp-btn vp-btn--ghost vp-btn--sm" @click="editingRemoteName = ''">取消</button>
            </template>
            <template v-else>
              <span class="gp-remote-url" :title="r.url">{{ r.url }}</span>
              <button class="vp-btn vp-btn--ghost vp-btn--sm" title="编辑此远程 URL" @click="startEdit(r.name, r.url)">编辑</button>
              <button class="vp-btn vp-btn--ghost vp-btn--sm gp-btn-danger" title="删除此远程" @click="$emit('remove-remote', r.name)">删除</button>
            </template>
          </div>
        </div>
        ...
        <div class="gp-remote-add">
          ...
          <input v-model="newRemoteUrl" class="gp-input" placeholder="远程 URL" style="flex:1" @keyup.enter="addRemote" />
          <button class="vp-btn vp-btn--primary vp-btn--sm" :disabled="!newRemoteName || !newRemoteUrl.trim()" @click="addRemote">添加</button>
        </div>
