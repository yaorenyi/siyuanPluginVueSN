...
export function createFlashcardReadingTool(plugin?: any): FloatingTool {
  return {
    ...
    action: (plugin?: any) => {
      if (plugin) {
        // 通过切换 dock 的显示状态来打开/关闭
        const dock = document.querySelector('[data-type=flashcardreading-dock]')
        if (dock) {
          // 触发 dock 切换
          const dockElement = dock as any
          if (dockElement.fnSwitch) {
            dockElement.fnSwitch()
          }
        }
      }
    }
