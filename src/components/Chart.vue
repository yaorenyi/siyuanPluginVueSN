<template>
  <div ref="containerRef" :class="chartClasses" :style="chartStyle">
    <canvas ref="canvasRef" />
    <div v-if="loading" class="si-chart__loading">
      <div class="si-chart__spinner"></div>
    </div>
    <div v-if="!loading && !hasData" class="si-chart__empty">
      <slot name="empty">
        <p>{{ emptyText }}</p>
      </slot>
    </div>
...
const drawLineChart = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  const maxValue = Math.max(...props.data.map(d => d.value), props.options?.maxY || 0)
  const minValue = props.options?.minY || 0
  const valueRange = maxValue - minValue || 1

  // 绘制网格线
  if (props.options?.showGrid !== false) {
    ctx.strokeStyle = props.theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'
    ctx.lineWidth = 1
    for (let i = 0; i <= 5; i++) {
  ...
  if (chartWidth / props.data.length > 40) {
    ctx.fillStyle = props.theme === 'dark' ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.7)'
    ctx.font = '11px sans-serif'
    ctx.textAlign = 'center'
